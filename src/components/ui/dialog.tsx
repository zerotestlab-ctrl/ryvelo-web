...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-border bg-card p-0 shadow-lg duration-200",
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
        "rounded-xl sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col",
        className
      )}
      {...props}
    >
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse gap-2 border-t border-border bg-black/20 px-6 py-4 sm:flex-row sm:justify-end",
      className
    )}
    {...props}
  />
